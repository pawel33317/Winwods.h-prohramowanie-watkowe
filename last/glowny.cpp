#include <iostream>//ZDARZENIE DO POT 1
#include <windows.h>
#include <cmath>
#include<string.h>
#include <stdio.h>
...
int main(int argc, char* argv[]) // glowny
{
	...
	HANDLE hMojeEvent = CreateEvent(NULL, TRUE,FALSE, "MojeEvent");
	if(hMojeEvent==NULL){  printf("CreateEvent error:");  getchar(); return 1;}
	HANDLE hFile =(HANDLE)0xffffffff; //plik wymianyt moze byc tez jakis tekstowy np
	HANDLE hMapFile = CreateFileMapping(hFile,NULL,PAGE_READWRITE, 0,1024, "MojaMapp");
	...
	BOOL okpot1 = CreateProcess(0, cmd1, 0,0,TRUE,CREATE_NEW_CONSOLE,0,0,  &si0, &pi);
		...
		char Komunikat[33];int sizeF=sizeof Komunikat;
	//////////////////////////////	
	WaitForSingleObject(hMojeEvent, 80000);
